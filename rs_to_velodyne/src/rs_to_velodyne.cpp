...
void rsHandler_XYZI(sensor_msgs::PointCloud2 pc_msg) {
    ...
    for (int point_id = 0; point_id < pc->points.size(); ++point_id) {
        ...
        // new_point.time = pc->points[point_id].time;
        // remap ring id
        if (pc->height == 16) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 3) {
        ...
        if (std::strcmp("XYZI", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/added_point_cloud", 1, rsHandler_XYZI);
        } else if (std::strcmp("XYZIRT", argv[1]) == 0) {
            subRobosensePC = nh.subscribe("/added_point_cloud", 1, rsHandler_XYZIRT);
        } else {
            ROS_ERROR(argv[1]);
