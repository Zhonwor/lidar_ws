...
FusionPcNode::FusionPcNode(ros::NodeHandle& nh)
{
  ...
  sync_ = new message_filters::Synchronizer<MySyncPolicy>(MySyncPolicy(10), *pc_sub1, *pc_sub2, *pc_sub3, *pc_sub4);
  //注册消息同步回调函数
                            ...
                            const sensor_msgs::PointCloud2::ConstPtr& pc_raw4)
{
  ...
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_local_1(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::fromROSMsg(*pc_raw1, *pc_local_1);  //父坐标系点云
  //子坐标系点云
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_local_2(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_trans_2(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::fromROSMsg(*pc_raw2, *pc_local_2);
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_local_3(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_trans_3(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::fromROSMsg(*pc_raw3, *pc_local_3);
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_local_4(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_trans_4(new pcl::PointCloud<RsPointXYZIRT>());
  ...
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_fusion_local(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_fusion_external_bounds(new pcl::PointCloud<RsPointXYZIRT>());
  pcl::PointCloud<RsPointXYZIRT>::Ptr pc_fusion_internal_bounds(new pcl::PointCloud<RsPointXYZIRT>());
  ...
  *pc_fusion_local = *pc_local_1;  //初始化
  ...
  {
    case 4:
      *pc_fusion_local = *pc_fusion_local + *pc_trans_4;
    case 3:
      *pc_fusion_local = *pc_fusion_local + *pc_trans_3;
    case 2:
      *pc_fusion_local = *pc_fusion_local + *pc_trans_2;
      break;
    default:
      std::cout << "Error: There is no matching option for fusion_lidar_num.";
      break;
  }

  //---> 5.点云过滤直接对点云的X、Y、Z轴的点云坐标约束来进行滤波 ---
  //点云过滤直接对点云的X、Y、Z轴的点云坐标约束来进行滤波 ---
...
void FusionPcNode::loadTransParams(Eigen::Isometry3d& T, const load_params::ParamTf& trans_params)
{
                                  ...
                                  const load_params::ParamsBounds& params_bounds)
{
  ...
  *cloud = *input_pc;
  ...
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::GT, params_bounds.z_min)));
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("z", pcl::ComparisonOps::LT, params_bounds.z_max)));
  //   // 设置Y轴的限制范围
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::GT, params_bounds.y_max)));
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("y", pcl::ComparisonOps::LT, params_bounds.y_min)));
  //   // 设置X轴的限制范围
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::GT, params_bounds.x_max)));
  range_cond->addComparison(pcl::FieldComparison<pcl::PointXYZI>::ConstPtr(
      new pcl::FieldComparison<pcl::PointXYZI>("x", pcl::ComparisonOps::LT, params_bounds.x_min)));
